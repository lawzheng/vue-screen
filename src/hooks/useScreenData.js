...
export default function () {
  ...
  onMounted(() => {
    task = setInterval(() => {
      ...
      _riderData.orderData.data1.map(item => {
        return item + random(100)
      })
      _riderData.orderData.data2.map(item => {
        return item + random(100)
      })
      _riderData.rateData.data1.map(item => {
        return item + random(100)
      })
      _riderData.rateData.data2.map(item => {
        return item + random(100)
      })
      riderData.value = _riderData

      const _hotCategoryData = { ...hotCategoryData.value }
      _hotCategoryData.data1.data1.map(item => {
        return item + random(100)
      })
